...
{\bf Interestingly, \citet{thy13} and \citet{pob13} have found a similar feature in their simulations and data respectively. In the latter, this feature is described as ``edge brightening'' and attributed to the Galactic plane near the horizon. From their discussion, it is unclear what fraction of power in that feature arises from ubiquitous diffuse emission unlike in our {\it zenith} pointing where the Galactic plane is not expected to play a significant role. On the other hand, \citet{thy13} statistically modeled point sources below the confusion limit and attributed the feature to an increase in solid angles subtended by delay bins closer to the horizon limits thereby, enhancing the resulting integrated emission. This must be true for diffuse emission as well. Thus we argue the feature arises primarily due to an increase in sensitivity of long baselines to diffuse emission caused by foreshortening towards angles far off--axis, where the increased size scales (or solid angles) containing higher levels of integrated emission and spatial scales given by the inverse of foreshortened baseline vectors become matched}. 
...
Having examined the role the foregrounds play in determining the delay spectrum obtained with an interferometer array, we turn our attention to another crucial factor, namely, the power pattern of the antenna (refer to equations~\ref{eqn:obsvis} and \ref{eqn:delay-transform}). In the following discussion, we assume there are no random delay fluctuations in the MWA tile beamforming process. The model power pattern so obtained is different from the one we used earlier, but for the present discussion it still serves as a good reference (Figure~\ref{fig:power_pattern}). Since the previous model suffices, we do not plot the new power pattern model obtained with ideal beamforming. We also assume an ideal flat bandpass shape over the 30.72~MHz bandwidth unlike the flagging of the edges of the coarse sub--bands described earlier in this paper, while still applying a {\it Blackman--Harris} window function.

\subsection{Uniformly Illuminated Sky}\label{sec:USM}

The brightness distribution on the sky was fixed to be a constant equal to the mean over the whole sky at 185~MHz with no variation along frequency. This helps isolate the signatures of the antenna power pattern from those of the foregrounds. Figure~\ref{fig:noiseless-usm-delay-spectrum} shows the delay spectrum power of such a sky model with the model power pattern of an MWA tile. 
...
Although not identical, these delay spectra power bear a close resemblance to those obtained with the diffuse sky model (see Figure~\ref{fig:noiseless-dsm-delay-spectrum}). This indicates that the delay spectra power seen from diffuse foregrounds is primarily explained by the delay spectrum signatures of the underlying power pattern, with the additional variations coming from anisotropic deviations of the diffuse foreground model from a uniformly illuminated sky.
...
Branches of foreground power seen at positive delays and close to the negative horizon delay limit in the {\it off--zenith} pointing correspond to radiation received from the direction of the primary lobe and the far eastward sidelobe, respectively, in the power pattern. On the other hand, the symmetry of the delay spectrum around $\tau=0$ seen in the {\it zenith} pointing is as expected from a symmetric power pattern.

\subsection{Experiments with Antenna Geometry}\label{sec:antgeom}

\begin{figure*}[htb]
\centering
\subfloat[][Power patterns]{\label{fig:power-patterns}\includegraphics[width=\linewidth]{fig15a.eps}} \\
\subfloat[][Delay spectra power]{\label{fig:antennas-delay-spectra}\includegraphics[width=\linewidth]{fig15b.eps}}
\caption{}
\label{fig:multi-antenna}
\end{figure*}
