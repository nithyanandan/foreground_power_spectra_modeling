...
Detection of 21~cm emission of H{\sc i} from the epoch of reionization, at redshifts $z>6$, is limited primarily by foreground emission.  We investigate the chromatic signatures of an all--sky foreground model imprinted by an instrumental transfer function. Using the delay spectrum technique {\bf that maps the measurements to foreground object locations through signal delays between antenna pairs}, we demonstrate that the foreground signatures that have the largest impact on the H{\sc i} signal arise from power received far away from the primary field of view. Comparing data from recent Murchison Widefield Array observations with simulations {\bf broken down by types of emission}, we identify diffuse emission near the horizon as a significant contributing factor, even on wide antenna spacings {\bf which are usually not sensitive to diffuse emission}. For signals entering through the primary field of view, compact emission dominates the foreground contamination. These two mechanisms imprint a characteristic {\it pitchfork} signature on the ``foreground wedge'' in Fourier space, {\bf which is found to be generic to wide--field measurements}. Based on these results, we propose that selective down--weighting of data based on antenna spacing and time {\bf can} mitigate foreground contamination substantially (up to a {\bf factor $\sim 100$} with negligible loss of sensitivity). {\bf A dish--shaped antenna yields the most desirable features from a foreground contamination viewpoint, relative to a dipole or a phased array}. 
...
The brightness distribution on the sky was fixed to be a constant equal to the mean over the whole sky at 185~MHz with no spectral variation. This helps isolate the signatures of the antenna power pattern from those of the foregrounds.
